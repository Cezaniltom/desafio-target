...
function analisarFaturamento(dados) {
    ...
    const somaTotal = diasUteis.reduce((acc, dia) => acc + dia.valor, 0);
    const mediaMensal = somaTotal / diasUteis.length;
    const diasAcimaMedia = diasUteis.filter(dia => dia.valor > mediaMensal).length;
...
const resultado = analisarFaturamento(faturamentos);
...
console.log(`Dias com faturamento acima da média mensal: ${resultado.diasAcimaMedia}`);
